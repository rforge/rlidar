...
#'@param exclusion A single value from 0 to 1 that represents the percentage of pixel exclusion. E.g. a value of 0.5 will exclude all of the pixels for a single tree that has a height value of less than 50 percent of the maximum height from the same tree. Default value is 0.3. 
...
ForestCAS<-function(chm,loc,maxcrown=10,exclusion=0.3) {
  ...
  #i=11
  DF2raster<-function(h.mH, i,loc){
    #print(i)
    h.mHkl<-subset(h.mH,h.mH[,4]==levels(factor(h.mH[,4]))[i])
    
    #if (nrow(h.mHkl==1)==TRUE) {
    #  h.mHkl<-rbind(h.mHkl,c(h.mHkl[,1],h.mHkl[,2],h.mHkl[,3],h.mHkl[,4]))}
    ...
    rv<-sp.polys[!is.na(over(sp.polys,SpatialPoints(loc[i,]))),]
    #plot(rv)
    
    #plot(sp.polys)
    #rv@polygons
    sp.polys2 = SpatialPolygons(
      list(
        Polygons(
          list(
            rv@polygons[[1]]@Polygons[[1]]
          ),
          ID=i)))
  ...
  RpD.filter<-subset(RpD[,1:5],RpD[,1]>=0)
  ...
  for ( j in 1:nlevels(factor(h.mH[,4]))){
    #print(j)
    assign(paste0("SP.polys", j), DF2raster(h.mH,j,loc))
